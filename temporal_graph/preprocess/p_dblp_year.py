...
class DBLPPreprocessing(Preprocessing):
    def __init__(self, input, output, dataset_name, top_n_journal, start_year, end_year) -> None:
        super().__init__(input=input, output=output, dataset_name=dataset_name, top_n_journal=top_n_journal, start_year=start_year, end_year=end_year)
    ...
    def processing(self, dataset_name):
        ...
        # -- filter by year
        df_edge_feat["year"] = df_edge_feat["year"].astype(int)
        df_edge_feat = df_edge_feat[(df_edge_feat["year"] >= self.start_year) & (df_edge_feat["year"] <= self.end_year)]
        
        ohc = OneHotEncoder(handle_unknown='ignore', sparse_output=False, drop='first')
    ...
    def _output_folder(self, dataset_name) -> str:
        dataset_name = f"{dataset_name}_{self.start_year}-{self.end_year}"
    ...
    def _pyg_path(self, dataset_name) -> str:
        return os.path.join(self._output_folder(dataset_name), f"pyg_{dataset_name}_{self.start_year}-{self.end_year}.pt")
    
    def _node_feat_path(self, dataset_name) -> str:
        return os.path.join(self._output_folder(dataset_name), f"pyg_{dataset_name}_{self.start_year}-{self.end_year}_node_feat.pt")

if __name__ == "__main__":
    import argparse

    argparser = argparse.ArgumentParser()
    argparser.add_argument("--start_year", type=int, default=1936)
    argparser.add_argument("--end_year", type=int, default=2024)
    ...
    dblp = DBLPPreprocessing(input="raw", output="data", dataset_name=f"dblp", top_n_journal=15, start_year=start_year, end_year=end_year)
    dblp.save()