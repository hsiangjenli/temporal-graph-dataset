...
if __name__ == "__main__":
    ...
    def format_with_comma(value):
        try:
            ...
            text = value.upper()
            text = "{\\scriptsize \\textbf{" + text + "}}" 
            return text
    
    reports = []
    dataset = TemporalGraphDataset(root='data')

    for dataset_name in dataset.available_datasets:
        data, x, train_mask, val_mask, test_mask = dataset(dataset_name)
        graph_info = TemporalGraphInfo(x, data, train_mask, val_mask, test_mask)
        reports.append(graph_info.report())
    
    df = pd.DataFrame(reports, index=dataset.available_datasets)
    df = df.T

    df["Properties"] = reports[0].keys()

    # -- reoder columns --
    df = df[["Properties"] + dataset.available_datasets]

    formatters = {col: format_with_comma for col in df.columns}
    
    print(df.to_latex(float_format="{:.4f}".format, formatters=formatters, escape=False, index=False, column_format=f"l{'r'*len(dataset.available_datasets)}"))
    
    # print(graph_info.report(to_latex=True))
    # print(graph_info.inductive_links)
    # print(graph_info.inductive_links_pct)

    # print(graph_info.tranductive_links)
    # print(graph_info.tranductive_links_pct)
